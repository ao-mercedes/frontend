...
export const Body2: React.FC<{ device: Device }> = ({device}) => {
    ...
    const extDescriptions = pitch && pitch.ExteriorDescriptions ? pitch.ExteriorDescriptions : [];
    console.log(`pitch ${JSON.stringify(pitch)}`);
    console.log(`extDescriptions ${JSON.stringify(extDescriptions)}`);
    console.log(`extDescriptions length ${JSON.stringify(extDescriptions.length)}`);
    ...
    return <div className="ao-body2-container" style={{backgroundColor: COLORS.CHARCOAL_SLATE,}}>
        <PageA4 device={device}
                style={{}}>
            <div
                style={{
                    marginTop: "35px",
